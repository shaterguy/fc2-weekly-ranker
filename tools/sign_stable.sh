...
cleanup() {
  ...
  trap - EXIT
  if (( rc != 0 )); then
    echo "sign_stable failed at stage=$STAGE" >&2
  fi
  rm -rf "$TMP"
  exit "$rc"
}
...
python3 "$ROOT/tools/derive_stable_signing_identity.py" \
  --pass-file "$PASS_FILE" \
  --out-key "$TMP/fc2-weekly-ranker-stable.pk8" \
  --out-cert "$TMP/fc2-weekly-ranker-stable-cert.pem" \
  > "$TMP/cert-sha256.txt"
...
STAGE=verify-domain-separation
[[ "$DERIVED_CERT_SHA256" != "$PINNED_TEST_CERT_SHA256" ]] || { echo "STABLE certificate must differ from TEST certificate" >&2; exit 1; }

STAGE=apk-sign
rm -f "$OUTPUT"
"$APKSIGNER" sign \
  --key "$TMP/fc2-weekly-ranker-stable.pk8" \
  --cert "$TMP/fc2-weekly-ranker-stable-cert.pem" \
  --min-sdk-version 29 \
...
if expected not in fingerprints:
    raise SystemExit("signed certificate does not match derived STABLE certificate")
print(expected)
PY
)"
[[ "$SIGNED_CERT_SHA256" == "$DERIVED_CERT_SHA256" ]] || { echo "signed certificate does not match derived STABLE certificate" >&2; exit 1; }
