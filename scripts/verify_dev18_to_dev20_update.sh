...
CURRENT_APK="${1:?usage: verify_dev18_to_dev20_update.sh <current-signed-apk>}"
...
if ! grep -Fq "package: name='$DEV_PACKAGE' versionCode='44' versionName='0.2.0-dev20'" <<<"$CURRENT_BADGING"; then
  echo 'ERROR: current TEST APK identity is not DEV20/versionCode 44.' >&2
  exit 1
fi
...
NEW_DB_DIR="$RUNNER_TEMP/dev20-update-db"
mkdir -p "$NEW_DB_DIR"
...
if con.execute("SELECT COUNT(*) FROM posts WHERE id='dev18-update-marker'").fetchone()[0] != 1:
    raise SystemExit("post marker lost during DEV18 to DEV20 update")
if con.execute("SELECT COUNT(*) FROM favorites WHERE postId='dev18-update-marker'").fetchone()[0] != 1:
    raise SystemExit("favorite marker lost during DEV18 to DEV20 update")
if con.execute("SELECT COUNT(*) FROM videos WHERE id='dev18-update-video'").fetchone()[0] != 1:
    raise SystemExit("video marker lost during DEV18 to DEV20 update")
...
if download != (456, 2000, 888, 3):
    raise SystemExit(f"download marker changed during DEV18 to DEV20 update: {download}")
con.close()
PY
