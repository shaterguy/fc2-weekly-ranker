...
if [[ ! "$DEV27_CERT_SHA256" =~ ^[0-9a-f]{64}$ ]]; then
  echo "ERROR: pinned DEV27 signer digest is malformed: $DEV27_CERT_SHA256" >&2
  exit 1
fi
...
con.execute("INSERT OR REPLACE INTO downloads(videoId,status,contentUri,downloadedBytes,totalBytes,errorCode,updatedAtEpochMillis,enqueueOrder,retryCount) VALUES(?,?,?,?,?,?,?,?,?)", ("dev27-update-video", "PAUSED", None, 456, 2000, None, 50, 888, 3))
con.commit()
...
echo 'DEV27 to DEV28 in-place TEST update verified.'
