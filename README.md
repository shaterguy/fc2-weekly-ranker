...
- Source version: `0.1.0-dev14`, `versionCode=14`. Runtime source remains based on the last known working `v0.1.0-dev3` baseline.
...
- Posting timestamps use the proven dev12 text path. Recommendation parsing is isolated from it and supports explicit labels plus the live metadata metric sequence before the posting timestamp. The ranking snapshot revision forces dev12 zero-count rows to refresh once.
