...
CURRENT_APK="${1:?usage: verify_dev35_to_dev36_update.sh <current-signed-apk>}"
...
OLD="$RUNNER_TEMP/fc2-dev35-source"
PASS_FILE="$RUNNER_TEMP/fc2-dev35-signing-pass"
...
grep -Fq "package: name='$DEV_PACKAGE' versionCode='60' versionName='0.2.0-dev36'" <<<"$CURRENT_BADGING"
"$BT/apksigner" verify --min-sdk-version 29 "$CURRENT_APK"

git fetch --no-tags origin refs/heads/v0.2.0-dev35:refs/remotes/origin/v0.2.0-dev35
git worktree prune
git worktree add --detach "$OLD" refs/remotes/origin/v0.2.0-dev35
...
test -s "$OLD_UNSIGNED"
OLD_ALIGNED="$RUNNER_TEMP/fc2-weekly-ranker-test-v0.2.0-dev35-aligned.apk"
OLD_APK="$RUNNER_TEMP/fc2-weekly-ranker-test-v0.2.0-dev35.apk"
...
grep -Fq "package: name='$DEV_PACKAGE' versionCode='59' versionName='0.2.0-dev35'" <<<"$OLD_BADGING"
...
OLD_CERT_SHA256="$(extract_cert_sha256 "$OLD_APK" fc2-dev35)"
CURRENT_CERT_SHA256="$(extract_cert_sha256 "$CURRENT_APK" fc2-dev36)"
...
if [[ "$REUSE_ADB" == '1' ]]; then
  ...
  export ANDROID_AVD_HOME="$RUNNER_TEMP/android-avd-dev35-update"
  rm -rf "$ANDROID_AVD_HOME"
  mkdir -p "$ANDROID_AVD_HOME"
  sdkmanager "platform-tools" "emulator" "system-images;android-29;google_apis;x86_64" >/dev/null
  echo no | avdmanager create avd --force --name fc2-dev35-update --package "system-images;android-29;google_apis;x86_64" --device pixel --path "$ANDROID_AVD_HOME/fc2-dev35-update.avd" >/dev/null
  "$ANDROID_HOME/emulator/emulator" -avd fc2-dev35-update -no-window -noaudio -no-boot-anim -no-snapshot -gpu swiftshader_indirect >"$RUNNER_TEMP/fc2-dev35-update-emulator.log" 2>&1 &
...
adb shell "run-as $DEV_PACKAGE sh -c 'mkdir -p files && printf dev35-to-dev36-marker > files/dev36-update-marker.txt'"

OLD_DB_DIR="$RUNNER_TEMP/dev35-update-db"
mkdir -p "$OLD_DB_DIR"
...
con.execute("DELETE FROM favorites WHERE postId='dev35-update-marker'")
con.execute("DELETE FROM posts WHERE id='dev35-update-marker'")
con.execute(
    ...
    ("dev35-update-marker", "https://example.test/dev35-update", "dev35-update-marker", 1, 7, 7.0, "dev35-update", 2, "FC2"),
)
con.execute("INSERT INTO favorites(postId,createdAtEpochMillis) VALUES(?,?)", ("dev35-update-marker", 3))
con.commit()
...
[[ "$(adb shell "run-as $DEV_PACKAGE cat files/dev36-update-marker.txt" | tr -d '\r')" == 'dev35-to-dev36-marker' ]]

NEW_DB_DIR="$RUNNER_TEMP/dev36-update-db"
mkdir -p "$NEW_DB_DIR"
...
post = con.execute(
    "SELECT title,recommendationCount,sourceKey FROM posts WHERE id='dev35-update-marker'"
).fetchone()
if post != ("dev35-update-marker", 7, "FC2"):
    raise SystemExit(f"DEV35 post marker changed during DEV36 update: {post}")
if con.execute("SELECT COUNT(*) FROM favorites WHERE postId='dev35-update-marker'").fetchone()[0] != 1:
    raise SystemExit("DEV35 favorite marker lost during DEV36 update")
con.close()
PY

echo 'DEV35 to DEV36 in-place TEST update, signer continuity, file retention, and ranker DB retention verified.'
