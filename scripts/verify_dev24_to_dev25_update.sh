...
CURRENT_APK="${1:?usage: verify_dev24_to_dev25_update.sh <current-signed-apk>}"
...
if ! grep -Fq "package: name='$DEV_PACKAGE' versionCode='49' versionName='0.2.0-dev25'" <<<"$CURRENT_BADGING"; then
  echo 'ERROR: current TEST APK identity is not DEV25/versionCode 49.' >&2
  exit 1
fi
...
NEW_DB_DIR="$RUNNER_TEMP/dev25-update-db"
mkdir -p "$NEW_DB_DIR"
...
version = con.execute("PRAGMA user_version").fetchone()[0]
if version != 3:
    raise SystemExit(f"expected DEV25 database v3, got {version}")
if con.execute("SELECT COUNT(*) FROM posts WHERE id='dev24-update-marker'").fetchone()[0] != 1:
    raise SystemExit("post marker lost during DEV24 to DEV25 update")
if con.execute("SELECT COUNT(*) FROM favorites WHERE postId='dev24-update-marker'").fetchone()[0] != 1:
    raise SystemExit("favorite marker lost during DEV24 to DEV25 update")
if con.execute("SELECT COUNT(*) FROM videos WHERE id='dev24-update-video'").fetchone()[0] != 1:
    raise SystemExit("video marker lost during DEV24 to DEV25 update")
...
if download != (456, 2000, 888, 3):
    raise SystemExit(f"download marker changed during DEV24 to DEV25 update: {download}")
con.close()
PY

adb shell dumpsys package "$STABLE_PACKAGE" | grep -Fq 'versionName=0.1.0'
adb shell dumpsys package "$DEV_PACKAGE" | grep -Fq 'versionName=0.2.0-dev25'
