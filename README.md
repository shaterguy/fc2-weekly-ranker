...
- Historical TEST lineage starts at `v0.1.0-dev1`; the current development target is `v0.2.0-dev26`.
- Source version: `0.2.0-dev26`, `versionCode=50`.
...
- External player delegation exposes only the opaque `content://` handle, resolved MIME type, and minimum URI read/prefix grants. The upstream URL, session cookies, `Referer`, user-agent overrides, authorization values, and full request headers remain inside the app/provider path.
- The external stream provider is non-exported and read-only; opaque session tokens are bounded by the provider session lifecycle rather than being reusable upstream URLs.
