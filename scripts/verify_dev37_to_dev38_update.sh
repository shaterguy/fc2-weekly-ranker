...
CURRENT_APK="${1:?usage: verify_dev37_to_dev38_update.sh <current-signed-apk>}"
...
OLD="$RUNNER_TEMP/fc2-dev37-source"
PASS_FILE="$RUNNER_TEMP/fc2-dev37-signing-pass"
...
grep -Fq "package: name='$DEV_PACKAGE' versionCode='62' versionName='0.2.0-dev38'" <<<"$CURRENT_BADGING"
"$BT/apksigner" verify --min-sdk-version 29 "$CURRENT_APK"

git fetch --no-tags origin refs/heads/v0.2.0-dev37:refs/remotes/origin/v0.2.0-dev37
git worktree prune
git worktree add --detach "$OLD" refs/remotes/origin/v0.2.0-dev37
...
test -s "$OLD_UNSIGNED"
OLD_ALIGNED="$RUNNER_TEMP/fc2-weekly-ranker-test-v0.2.0-dev37-aligned.apk"
OLD_APK="$RUNNER_TEMP/fc2-weekly-ranker-test-v0.2.0-dev37.apk"
...
grep -Fq "package: name='$DEV_PACKAGE' versionCode='61' versionName='0.2.0-dev37'" <<<"$OLD_BADGING"
...
if [[ "$REUSE_ADB" == '1' ]]; then
  ...
  export ANDROID_AVD_HOME="$RUNNER_TEMP/android-avd-dev37-update"
  rm -rf "$ANDROID_AVD_HOME"
  mkdir -p "$ANDROID_AVD_HOME"
  sdkmanager "platform-tools" "emulator" "system-images;android-29;google_apis;x86_64" >/dev/null
  echo no | avdmanager create avd --force --name fc2-dev37-update --package "system-images;android-29;google_apis;x86_64" --device pixel --path "$ANDROID_AVD_HOME/fc2-dev37-update.avd" >/dev/null
  "$ANDROID_HOME/emulator/emulator" -avd fc2-dev37-update -no-window -noaudio -no-boot-anim -no-snapshot -gpu swiftshader_indirect >"$RUNNER_TEMP/fc2-dev37-update-emulator.log" 2>&1 &
...
con.execute(
    ...
    ("dev37-update-marker", "https://example.test/dev37-update", "dev37-update-marker", 1, 7, 7.0, "dev37-update", 2, "FC2"),
)
con.execute("INSERT INTO favorites(postId,createdAtEpochMillis) VALUES(?,?)", ("dev37-update-marker", 3))
con.commit()
...
[[ "$(adb shell "run-as $DEV_PACKAGE cat files/dev38-update-marker.txt" | tr -d '\r')" == 'dev37-to-dev38-marker' ]]

NEW_DB_DIR="$RUNNER_TEMP/dev38-update-db"
mkdir -p "$NEW_DB_DIR"
...
post = con.execute(
    "SELECT title,recommendationCount,sourceKey FROM posts WHERE id='dev37-update-marker'"
).fetchone()
if post != ("dev37-update-marker", 7, "FC2"):
    raise SystemExit(f"DEV37 post marker changed during DEV38 update: {post}")
if con.execute("SELECT COUNT(*) FROM favorites WHERE postId='dev37-update-marker'").fetchone()[0] != 1:
    raise SystemExit("DEV37 favorite marker lost during DEV38 update")
con.close()
PY

echo 'DEV37 to DEV38 in-place TEST update, signer continuity, file retention, and ranker DB retention verified.'
