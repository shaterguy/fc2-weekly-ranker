...
CURRENT_APK="${1:?usage: verify_dev34_to_dev35_update.sh <current-signed-apk>}"
...
OLD="$RUNNER_TEMP/fc2-dev34-source"
PASS_FILE="$RUNNER_TEMP/fc2-dev34-signing-pass"
...
grep -Fq "package: name='$DEV_PACKAGE' versionCode='59' versionName='0.2.0-dev35'" <<<"$CURRENT_BADGING"
"$BT/apksigner" verify --min-sdk-version 29 "$CURRENT_APK"

git fetch --no-tags origin refs/heads/v0.2.0-dev34:refs/remotes/origin/v0.2.0-dev34
git worktree prune
git worktree add --detach "$OLD" refs/remotes/origin/v0.2.0-dev34
...
test -s "$OLD_UNSIGNED"
OLD_ALIGNED="$RUNNER_TEMP/fc2-weekly-ranker-test-v0.2.0-dev34-aligned.apk"
OLD_APK="$RUNNER_TEMP/fc2-weekly-ranker-test-v0.2.0-dev34.apk"
...
grep -Fq "package: name='$DEV_PACKAGE' versionCode='58' versionName='0.2.0-dev34'" <<<"$OLD_BADGING"
...
OLD_CERT_SHA256="$(extract_cert_sha256 "$OLD_APK" fc2-dev34)"
CURRENT_CERT_SHA256="$(extract_cert_sha256 "$CURRENT_APK" fc2-dev35)"
...
if [[ "$REUSE_ADB" == '1' ]]; then
  ...
  export ANDROID_AVD_HOME="$RUNNER_TEMP/android-avd-dev34-update"
  rm -rf "$ANDROID_AVD_HOME"
  mkdir -p "$ANDROID_AVD_HOME"
  sdkmanager "platform-tools" "emulator" "system-images;android-29;google_apis;x86_64" >/dev/null
  echo no | avdmanager create avd --force --name fc2-dev34-update --package "system-images;android-29;google_apis;x86_64" --device pixel --path "$ANDROID_AVD_HOME/fc2-dev34-update.avd" >/dev/null
  "$ANDROID_HOME/emulator/emulator" -avd fc2-dev34-update -no-window -noaudio -no-boot-anim -no-snapshot -gpu swiftshader_indirect >"$RUNNER_TEMP/fc2-dev34-update-emulator.log" 2>&1 &
...
adb shell "run-as $DEV_PACKAGE sh -c 'mkdir -p files && printf dev34-to-dev35-marker > files/dev35-update-marker.txt'"

OLD_DB_DIR="$RUNNER_TEMP/dev34-update-db"
mkdir -p "$OLD_DB_DIR"
...
if con.execute("PRAGMA user_version").fetchone()[0] != 4:
    raise SystemExit("DEV34 ranker database is not v4")
con.execute("PRAGMA foreign_keys=ON")
con.execute("DELETE FROM favorites WHERE postId='dev34-update-marker'")
con.execute("DELETE FROM posts WHERE id='dev34-update-marker'")
con.execute(
    ...
    ("dev34-update-marker", "https://example.test/dev34-update", "dev34-update-marker", 1, 7, 7.0, "dev34-update", 2, "FC2"),
)
con.execute("INSERT INTO favorites(postId,createdAtEpochMillis) VALUES(?,?)", ("dev34-update-marker", 3))
con.commit()
...
adb shell am force-stop "$DEV_PACKAGE"
sleep 1
adb shell dumpsys package "$DEV_PACKAGE" | grep -Fq 'versionName=0.2.0-dev35'
[[ "$(adb shell "run-as $DEV_PACKAGE cat files/dev35-update-marker.txt" | tr -d '\r')" == 'dev34-to-dev35-marker' ]]

NEW_DB_DIR="$RUNNER_TEMP/dev35-update-db"
mkdir -p "$NEW_DB_DIR"
...
if con.execute("PRAGMA user_version").fetchone()[0] != 4:
    raise SystemExit("DEV35 ranker database is not v4")
post = con.execute(
    "SELECT title,recommendationCount,sourceKey FROM posts WHERE id='dev34-update-marker'"
).fetchone()
if post != ("dev34-update-marker", 7, "FC2"):
    raise SystemExit(f"DEV34 post marker changed during DEV35 update: {post}")
if con.execute("SELECT COUNT(*) FROM favorites WHERE postId='dev34-update-marker'").fetchone()[0] != 1:
    raise SystemExit("DEV34 favorite marker lost during DEV35 update")
con.close()
PY

echo 'DEV34 to DEV35 in-place TEST update, signer continuity, file retention, and ranker DB retention verified.'
