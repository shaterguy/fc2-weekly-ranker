...
CURRENT_APK="${1:?usage: verify_dev28_to_dev29_update.sh <current-signed-apk>}"
...
if ! grep -Fq "package: name='$DEV_PACKAGE' versionCode='53' versionName='0.2.0-dev29'" <<<"$CURRENT_BADGING"; then
  echo 'ERROR: current TEST APK identity is not DEV29/versionCode 53.' >&2
  exit 1
fi
...
CURRENT_CERT_SHA256="$(extract_cert_sha256 "$CURRENT_APK" fc2-dev29)"
...
if [[ "$CURRENT_CERT_SHA256" != "$PINNED_TEST_CERT_SHA256" ]]; then
  echo "ERROR: DEV29 signer mismatch: $CURRENT_CERT_SHA256" >&2
  exit 1
fi
...
adb shell "run-as $DEV_PACKAGE sh -c 'mkdir -p files && printf dev28-update-marker > files/dev29-update-marker.txt'"
MARKER_BEFORE="$(adb shell "run-as $DEV_PACKAGE cat files/dev29-update-marker.txt" | tr -d '\r')"
...
adb shell dumpsys package "$DEV_PACKAGE" | grep -Fq 'versionName=0.2.0-dev29'
MARKER_AFTER="$(adb shell "run-as $DEV_PACKAGE cat files/dev29-update-marker.txt" | tr -d '\r')"
if [[ "$MARKER_AFTER" != 'dev28-update-marker' ]]; then
  echo 'ERROR: DEV28 application data marker did not survive DEV29 in-place update.' >&2
  exit 1
fi

echo 'DEV28 to DEV29 in-place TEST update and stable co-install verified.'
